...
const addMenuItem = TryCatch(async (req, res) => {
    if (!req.user) {
...
const getAllItems = TryCatch(async (req, res) => {
    const { id } = req.params;
    if (!id) {
        return res.status(400).json({
...
const deleteMenuItem = TryCatch(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            message: "Please login",
        });
...
const toggleMenuItemAvailability = TryCatch(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            message: "Please login",
        });
