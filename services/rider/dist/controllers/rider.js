...
const addRiderProfile = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
...
const fetchMyProfile = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({
            message: "Unauthorized"
...
const toogleRiderAvailability = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({
            message: "Unauthorized"
...
const acceptOrder = TryCatch(async (req, res) => {
...
const fetchMyCurrentOrder = TryCatch(async (req, res) => {
    const riderUserId = req.user?._id;
    if (!riderUserId) {
        return res.status(401).json({
            message: "Please Login."
...
const fetchRiderHistory = TryCatch(async (req, res) => {
    const riderUserId = req.user?._id;
    if (!riderUserId) {
        return res.status(401).json({
            message: "Please login.",
    ...
    try {
        const { data } = await axios.get(`${process.env.RESTAURANT_SERVICE}/api/order/rider/${rider._id}/history`, {
            params: {
                range,
            },
            headers: {
